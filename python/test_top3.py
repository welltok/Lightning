...
# it produces an output indicating accuracy and recalls
...
def test(inputfile, classifierid, username, password):
	...
	for entry in test_data:
		...
		top3_class = []
		top3_conf = []
		for top_classes in json_response['classes'][0:3]:
			if top_classes['class_name'] == correct_class:
				top_3 +=1
		for top_classes in json_response['classes'][0:1]:
	
			top3_class.append(top_classes['class_name'])

		top3_classes[correct_class].append(top3_class)
	...
	percent_correct = top_3 * 100 / total
	if percent_correct > 90:
		print "num of times in top 3: " + str(top_3) + " (percentage: " + colored(str(percent_correct) + "%)", 'green')
	else:
		print "num of times in top 3: " + str(top_3) + " (percentage: " + colored(str(percent_correct) + "%)", 'red')
	...
	for k in all_classes:
		...
		top_confusions = count_lol(top3_classes[k])[0:3]
