...
def test(inputfile, classifierid, username, password, dumpresultfile, env):
    response_with_truth = []
    f = open(inputfile, 'r+')
    ...
    for entry in test_data:
        ...
        if dumpresultfile != None:
            with io.open(dumpresultfile + '.json', 'w', encoding='utf-8') as f:
                f.write(unicode(
